package cryptobox

import (
	"bytes"
	"testing"
)

/*
func TestCreateOpen(t *testing.T) {
	pk, sk := KeyPair()
	m := []byte("Hello world")
	n := make([]byte, 24)
	c := Create(m, n, pk, sk)
	dec := Open(c, n, pk, sk)
	if dec == nil {
		t.Errorf("failed to decrypt")
	}
	if string(dec) != string(m) {
		t.Errorf("Open: expected %q, got %q", string(m), string(dec))
	}
}
*/

var alicesk = [32]byte{
	0x77,0x07,0x6d,0x0a,0x73,0x18,0xa5,0x7d,
	0x3c,0x16,0xc1,0x72,0x51,0xb2,0x66,0x45,
	0xdf,0x4c,0x2f,0x87,0xeb,0xc0,0x99,0x2a,
	0xb1,0x77,0xfb,0xa5,0x1d,0xb9,0x2c,0x2a}

var bobpk = [32]byte{
	0xde,0x9e,0xdb,0x7d,0x7b,0x7d,0xc1,0xb4,
	0xd3,0x5b,0x61,0xc2,0xec,0xe4,0x35,0x37,
	0x3f,0x83,0x43,0xc8,0x5b,0x78,0x67,0x4d,
	0xad,0xfc,0x7e,0x14,0x6f,0x88,0x2b,0x4f}

var alicenonce = [24]byte{
	0x69,0x69,0x6e,0xe9,0x55,0xb6,0x2b,0x73,
	0xcd,0x62,0xbd,0xa8,0x75,0xfc,0x73,0xd6,
	0x82,0x19,0xe0,0x03,0x6b,0x7a,0x0b,0x37}

var alicem = []byte{
	}

var alicec = []byte{
	}

func TestAliceBob(t *testing.T) {
	c := Create(alicem, &alicenonce, &bobpk, &alicesk)
	if !bytes.Equal(c, alicec) {
		t.Errorf("Alice's box differs from test vector")
	}
}

// Benchmarks.

var (
	pka, ska [32]byte
	pk = &pka
	sk = &ska
	noncearr  [NonceSize]byte
	nonce = &noncearr
	msg    = make([]byte, 8<<10)
)

func init() {
	KeyPair(pk, sk)
}

func BenchmarkBefore(b *testing.B) {
	var k [32]byte
	for i := 0; i < b.N; i++ {
		Before(&k, pk, sk)
	}
}

func BenchmarkCreate50(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Create(msg[:50], nonce, pk, sk)
	}
	b.SetBytes(50)
}

func BenchmarkCreate1K(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Create(msg[:1024], nonce, pk, sk)
	}
	b.SetBytes(1024)
}

func BenchmarkCreate8K(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Create(msg, nonce, pk, sk)
	}
	b.SetBytes(int64(len(msg)))
}

func BenchmarkOpenValid1K(b *testing.B) {
	b.StopTimer()
	c := Create(msg[:1024], nonce, pk, sk)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		Open(c, nonce, pk, sk)
	}
	b.SetBytes(int64(len(c)))
}

func BenchmarkOpenValid8K(b *testing.B) {
	b.StopTimer()
	c := Create(msg, nonce, pk, sk)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		Open(c, nonce, pk, sk)
	}
	b.SetBytes(int64(len(c)))
}

func BenchmarkOpenInvalid1K(b *testing.B) {
	b.StopTimer()
	c := Create(msg[:1024], nonce, pk, sk)
	c[100] = 1 // make cipher text non-verifiable.
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		Open(c, nonce, pk, sk)
	}
	b.SetBytes(int64(len(c)))
}

func BenchmarkOpenInvalid8K(b *testing.B) {
	b.StopTimer()
	c := Create(msg, nonce, pk, sk)
	c[100] = 1 // make cipher text non-verifiable.
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		Open(c, nonce, pk, sk)
	}
	b.SetBytes(int64(len(c)))
}
